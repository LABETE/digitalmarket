import os
import shutil
from PIL import Image
import random

from django.conf import settings
from django.core.files import File
from django.core.files.storage import FileSystemStorage
from django.core.urlresolvers import reverse
from django.db import models
from django.db.models.signals import pre_save, post_save
from django.utils.text import slugify

from sellers.models import SellerAccount


def download_media_location(instance, filename):
    return "{0}/{1}".format(instance.slug, filename)


class Product(models.Model):
    seller = models.ForeignKey(SellerAccount)
    media = models.ImageField(blank=True,
                             null=True,
                             upload_to=download_media_location,
                             storage=FileSystemStorage(
                                 location=settings.PROTECTED_ROOT))
    title = models.CharField(max_length=30)
    slug = models.SlugField(unique=True, blank=True)
    description = models.TextField()
    price = models.DecimalField(
        max_digits=100, decimal_places=2, default=9.99, null=True, blank=True)
    price_active = models.BooleanField(default=True)
    sale_price = models.DecimalField(max_digits=100,
                                     decimal_places=2,
                                     default=6.99,
                                     null=True,
                                     blank=True)

    def __str__(self):
        return self.title

    def get_absolute_url(self):
        return reverse("products:detail_slug", kwargs={"slug": self.slug})

    def get_edit_url(self):
        return reverse("sellers:product_update", kwargs={"pk": self.id})

    def get_download(self):
        return reverse("products:download_slug", kwargs={"slug": self.slug})

    @property
    def get_price(self):
        if self.sale_price and self.price_active:
            return self.sale_price
        return self.price

    def get_html_price(self):
        if self.sale_price and self.price_active:
            return "<p><span>{0}</span> <span style='color:red;text-decoration:line-through;'>{1}</span></p>".format(self.sale_price, self.price)
        return self.price


def create_slug(instance, new_slug=None):
    slug_name = slugify(instance.title)
    slug = slugify(instance.title)
    if new_slug is not None:
        slug = new_slug
    qs = Product.objects.filter(slug=slug)
    exists = qs.exists()
    if exists:
        slug_num = slug[::-1].split("-", 1)
        if len(slug_num) == 2:
            slug_num = int(slug_num[0]) + 1
        else:
            slug_num = 1
        new_slug = "{0}-{1}".format(slug_name, slug_num)
        return create_slug(instance, new_slug=new_slug)
    else:
        return slug


def product_pre_save_receiver(sender, instance, *args, **kwargs):
    if not instance.slug:
        instance.slug = create_slug(instance)

pre_save.connect(product_pre_save_receiver, sender=Product)


def thumbnail_location(instance, filename):
    return "{0}/{1}".format(instance.product.slug, filename)


THUMBNAIL_CHOICES = (
    ('hd', 'HD'),
    ('sd', 'SD'),
    ('micro', 'Micro'),
)


class Thumbnail(models.Model):
    product = models.ForeignKey(Product)
    type = models.CharField(max_length=20, choices=THUMBNAIL_CHOICES, default="hd")
    height = models.CharField(max_length=20, null=True, blank=True)
    width = models.CharField(max_length=20, null=True, blank=True)
    media = models.ImageField(
        width_field="width",
        height_field="height",
        blank=True,
        null=True,
        upload_to=thumbnail_location)

    def __str__(self):
        if self.media:
            return self.media.path


def create_new_thumb(media_path, instance, owner_slug, size):
    filename = os.path.basename(media_path)
    thumb = Image.open(media_path)
    thumb.thumbnail(size, Image.ANTIALIAS)
    temp_loc = "{0}/{1}/tmp".format(settings.MEDIA_ROOT, owner_slug)
    if not os.path.exists(temp_loc):
        os.makedirs(temp_loc)
    temp_file_path = os.path.join(temp_loc, filename)
    if os.path.exists(temp_file_path):
        temp_path = os.path.join(temp_loc, "{0}".format(random.random()))
        os.makedirs(temp_path)
        temp_file_path = os.path.join(temp_path, filename)
    temp_image = open(temp_file_path, "w")
    thumb.save(temp_image, format="JPEG")
    thumb_data = open(temp_file_path, "rb")
    thumb_file = File(thumb_data)
    instance.media.save(filename, thumb_file)
    shutil.rmtree(temp_loc, ignore_errors=True)


def product_post_save_receiver(sender, instance, created, *args, **kwargs):
    if instance.media:
        hd, hd_created = Thumbnail.objects.get_or_create(product=instance, type="hd")
        sd, sd_created = Thumbnail.objects.get_or_create(product=instance, type="sd")
        micro, micro_created = Thumbnail.objects.get_or_create(product=instance, type="micro")
        hd_max = (500, 500)
        sd_max = (350, 350)
        micro_max = (150, 150)

        media_path = instance.media.path
        owner_slug = instance.slug
        if hd_created:
            create_new_thumb(media_path, hd, owner_slug, hd_max)

        if sd_created:
            create_new_thumb(media_path, sd, owner_slug, sd_max)

        if micro_created:
            create_new_thumb(media_path, micro, owner_slug, micro_max)
            
post_save.connect(product_post_save_receiver, sender=Product)


class MyProducts(models.Model):
    user = models.OneToOneField(settings.AUTH_USER_MODEL)
    products = models.ManyToManyField(Product)

    def __str__(self):
        return "{0}".format(self.products.count())

    class Meta:
        verbose_name = 'My Products'
        verbose_name_plural = 'My Products'


class ProductRating(models.Model):
    user = models.OneToOneField(settings.AUTH_USER_MODEL)
    product = models.ForeignKey(Product)
    rating = models.IntegerField(null=True, blank=True)
    verified = models.BooleanField(default=False)

    def __str__(self):
        return "{0}".format(self.rating)

class CuratedProducts(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL)
    section_name = models.CharField(max_length=120)
    products = models.ManyToManyField(Product, blank=True)
    active = models.BooleanField(default=True)

    class Meta:
        verbose_name = "Curated Products"
        verbose_name_plural = "Curated Products"

    def __str__(self):
        return self.section_name